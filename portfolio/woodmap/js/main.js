...
$(document).ready(function () {
	$('.header__burger').click(function (event) {
		$('.header__burger, .menu__layout').toggleClass('active');
		$('body').toggleClass('lock');
	});
});
$('.menu__link,.header__btn').click(function (event) {
	$('.header__burger, .menu__layout').removeClass('active');
	$('body').removeClass('lock');
});
...
function initializeClock(id, endtime) {
	...
	function updateClock() {
		...
		month[10] = "Ноября";
		month[11] = "Декабря";
		var result = month[now.getMonth()];

		var year = now.getFullYear();
		var mmonth = now.getMonth();
		var eday = new Date(year, mmonth + 1, 0);
		var uday = eday.getDate();
		if (ttd > uday) {
			var tttd = uday;
		}
		else {
		...
		document.querySelector(".first-sale__title > span").textContent = tttd + ' ' + result;
	}
...
var deadline = "October 4 2020 00:00:00 GMT+0300";
initializeClock("countdown", deadline);
...
$(document).ready(function () {
	...
	$('.slider').on('afterChange', function (event, slick, currentSlide) {
		window.dataLayer.push({ 'event': 'swipe-slide' });
	});
	...
	$(function () {
		...
		$('.popup-form').submit(function () {
			$.ajax({
				...
				$('.popup').removeClass('active').fadeOut(300);
				$('.popup-thanks').delay(300).fadeIn(300).addClass('active');
				ym(64862182, 'reachGoal', 'form-send');
				window.dataLayer.push({ 'event': 'form-send' });
				// $('body').removeClass('lock');
				// history.pushState('', '', window.location.href.split('#')[0]);
